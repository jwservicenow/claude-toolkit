...
- **Where an existing row already covers the subject, edit that row in place.** Never add a
  ...
  section exists for.
- **If this session overturned a conclusion**, mark the old row `SUPERSEDED → <type> "<opening
  words of the replacing row>"` and keep it — the same syntax `/newplan`'s template uses. Never leave two live
  rows on one subject.
...
an unresolved problem → `D`; a tool → the `Tools:` line. **An orphaned row takes the date
