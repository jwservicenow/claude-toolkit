---
name: newsession-pro
description: Token flush for long conversations — when context is filling up or a topic is wrapping up, invoke /newsession-pro. Two modes: `/newsession-pro` (silent — writes the handoff file only, no output, no pre-flight check; the sole exception is the one flush that promotes an ad-hoc thread to a findings artifact, which says so once) and `/newsession-pro full` (also runs the urgent must-do-now check, then writes the file and prints its path). Optionally shaped by a runbook or planning file. Strictly user-invoked — never auto-triggers.
---

# /newsession-pro — Session handoff

Look at what actually happened in this conversation (this session only — not memory, not prior sessions).

**Strictly user-invoked.** Only activate when the user types `/newsession-pro`. Never auto-trigger.
...
The spec is `shared/skills/newplan-pro/record-controls.md` (`CANONICAL:record-controls`) — follow it,
...
bare `/newsession-pro` still outputs `<!-- no output -->` and nothing else; `full` still prints only
...
that grew out of a one-off question and never had a `/newplan-pro`. The rule is `record-controls.md`
...
A bare `/newsession-pro` prints this **instead of** `<!-- no output -->` on that one flush, and goes
...
`/newplan-pro` stays the user's decision and is a different one — this step only gives existing
...
A check that fails every session means `/newplan-pro` set the project up without these invariants, or
the artifacts were hand-edited around them. Say so in the handoff's Open threads, once.

## Step 3 — Save the handoff to disk

Save the generated handoff prompt as a standalone prompt file — this becomes the project's resume point. Mirror `/newplan-pro`'s naming:
- Write to the **current working directory** (the project being worked on) as `<topic>-prompt-YYYY-MM-DD.md` with today's date. Derive `<topic>` in this order, first match wins:
  ...
  2. The label of the `*-plan-*.md` in the cwd that **this session actually worked on** — strip the `-plan-YYYY-MM-DD.md` suffix and reuse the label verbatim, so the pair matches (`<label>-plan-2026-08-03.md` → `<label>-prompt-2026-08-03.md`). If none was worked on this session, skip to 3 — do not adopt a plan's label just because the file is present.
     ...
     **"Worked on" means executed against, not authored.** A plan this session *wrote* — via `/newplan-pro` or by hand — is a **product** of the session, not its subject. **Exclude it from candidacy**, unless it is the only candidate. Writing a plan is a normal way to end a session, so without this the handoff is filed under whatever was planned last rather than under what was done, and the real thread silently gets no handoff.

     If several plans remain, break the tie **deterministically, in this order**: (a) highest date in the filename; (b) still tied → most recently modified on disk (`ls -t`); (c) still tied → skip to 3 and use the directory name. Never pick between same-date plans by judgment — an arbitrary pick is what mis-files a handoff. Note that (b) is only safe *because* session-authored plans were already excluded: a file just written always has the newest mtime, so without the exclusion (b) would pick it every time.
...
The handoff prompt is the one exception to the working-artifacts rule — like the plan it points at, it stays flat in the project directory because it is the resume pointer. **Any other file this session created goes by `/newplan-pro`'s *Working artifacts* section** (`shared/skills/newplan-pro/SKILL.md`): `<topic>-<kind>-YYYY-MM-DD.md`, durable at the project root, ephemeral in `<project>/run/`. Do not restate that rule in the handoff — cite the artifacts by path and let the convention do the rest.
...
(A bare `/newsession-pro` never prints this line. Its entire visible reply is `<!-- no output -->`
...
3. **Say so.** Add one line as the last line of the file: `Cap: N lines against a soft cap of 120 — over by M. Kept: <what and why>.` In `full` mode, append the same overage to Step 4's single line. A bare `/newsession-pro` still prints nothing — the notice lives in the file, which is where it survives.
